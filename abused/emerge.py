...
class Emerge(object):
    ...
    def _sanitize(self, data):
        retv = ""
        if type(data) == str and data.find("\x1b") != -1:
            tmp = "".join(filter(lambda x: x in string.printable, data))
    ...
    def _parser(self, data=None):
        ...
        if len(ldata) > 0 and ldata[0] == "[":
            cat = {"category": None, "package": None, "version": None, "variable": None}
            ...
            for tkn in range(0, lmax):
                ...
                elif cat["package"] == False and not re.match(
                    "-[0-9]", "".join(ldata[tkn : tkn + 2])
                ):
                    pkg["package"] += ldata[tkn]
                elif cat["package"] == False and re.match(
                    "-[0-9]", "".join(ldata[tkn : tkn + 2])
                ):
                    cat["package"] = True
                    cat["version"] = False
    ...
    def _cmd(self, cmd, parse=True):
        ...
        if parse:
            ...
            buff = cmd_p.stdout.readline().decode("utf-8")
            while buff:
