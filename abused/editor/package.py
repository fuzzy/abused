    ...
    def __init__(self, pkg={}):
        ...
        try:
            if len(pkg["variables"].keys()) == 0:
                print(
                    "%s: This package (%s/%s) has no variables to edit."
                    % (Scale("INFO").bold().cyan(), pkg["category"], pkg["package"])
                )
                return
        except TypeError:
            print("%s: Not a valid package object" % Scale("ERROR").bold().red())
        ...
        self.pkg = pkg
        self.keys = list(pkg["variables"].keys())
        self.editing = ""
        self.edited = False
    ...
    def do_refresh(self, line=None):
        ...
        if len(self.keys) > 0:
            ...
            for k in range(0, len(self.keys)):
                print(
                    "%s: %s"
                    % (Scale(k).bold().red(), Scale(self.keys[k]).bold().yellow())
                )
            print(
                "\n%s:\n%s\n"
                % (
